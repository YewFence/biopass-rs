...
impl FaceAuth {
    ...
    fn check_anti_spoofing(
        ...
        if ai_enabled {
            ...
            if anti_spoofing.detect(face)?.spoof {
                return Ok(false);
            }
        }

        if ir_enabled && !self.check_ir_face_presence()? {
            return Ok(false);
        }
    ...
    fn check_ir_face_presence(&self) -> Result<bool, String> {
        ...
        Ok(!detector.detect(&frame)?.is_empty())
    }
}
