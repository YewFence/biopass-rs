...
安装好 [mise](https://mise.jdx.dev/getting-started.html) 后，先信任这个仓库的配置：

```bash
mise trust
```

**2. 可选的实验性 bootstrap 流程：**

`mise.toml` 在 `[bootstrap.packages]` 里声明了 Linux 系统依赖，在 `[tools]` 里声明了语言工具链。如果你安装的 [mise](https://mise.jdx.dev/getting-started.html) 版本支持实验性的 bootstrap 功能，可以让 mise 一步安装缺失的系统包，再安装固定版本的 Rust、Bun、Node、Vite+ 和 git-lfs：

```bash
mise bootstrap --update
```

在 Linux 上这可能会触发 `sudo` 密码提示。bootstrap 的系统包条目会按包管理器过滤，因此 Fedora 会忽略 `apt` 包，Ubuntu/Debian 会忽略 `dnf` 包。

因为 mise bootstrap 仍然是实验性功能，你也可以继续使用下面的手动系统依赖流程。
...
手动安装系统包后，再用 mise 安装语言工具链：

`mise.toml` 在 `[tools]` 里固定了 Rust、Bun、Node、Vite+ 和 git-lfs 的精确版本。无需单独跑 rustup 或 Bun 的安装脚本：
