...
This flow is handled by [Polkit](https://github.com/polkit-org/polkit). In some cases, biopass-rs authentication is not triggered by Polkit due to the strict policy on device accesses.
...
3. Reload systemd and restart polkit
    ...
    ```
4. Check the configuration in the biopass-rs UI. Please make sure to remove `pkexec`, `polkit-1` from the ignore service list ![alt text](images/ignore-services.png)
