...
1. **LED / 曝光预热** — IR 相机最初可能返回暗帧，因此 biopass-rs 在采集计数的帧之前，先通过拉取帧来预热流 `ir.warmup_delay_ms`（默认 300ms），直到传感器的曝光/增益收敛。
...
当且仅当 SPOOF 类没有胜出、real 分数达到 `anti_spoofing.ir.model.threshold`、且 real 分数严格大于 spoof 分数时，该帧才被接受为真。

每次认证尝试，biopass-rs 收集 3 张可用的 IR 人脸裁切图，要求其中至少 2 张通过活体检查 — 严格的多数投票。结构性失败（模型缺失、帧不可读、人脸过小、空间不匹配）始终被视为 spoof。IR 模型自身的 spoof 判定是可配置的：当 `ir.ir_model_hard_fail` 为 `true` 时，分类器的 spoof 判定也会使检查失败（fail-closed）；为 `false`（默认）时，分类器分数仅作诊断用途 — 高 spoof 分数会被记录，但本身不会拒绝本次尝试。其他检查（人脸存在、尺寸、空间匹配）无论何种情况都必须通过。

当 RGB 反欺骗模型与 IR 同时启用（`anti_spoofing.rgb`）时，**两者都必须独立通过**才会授予认证。RGB AI 反欺骗任务与 IR 活体任务串行执行。

`ir.auto_optimize_camera`（默认 `false`）将 RGB 相机的 V4L2 曝光和图像控制设置复用到 IR 相机上，有助于那些需要匹配曝光才能产生可用帧的 IR 传感器。
